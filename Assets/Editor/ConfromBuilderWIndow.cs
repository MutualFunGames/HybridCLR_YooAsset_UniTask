...
public class ConfromBuilderWIndow : EditorWindow
{
    /// <summary>
    /// 工程目录路径，Assets上一层
    /// </summary>
    ...
    {
        ConfromBuilderWIndow window =
            GetWindow<ConfromBuilderWIndow>("AssetBundle Builder", true, WindowsDefine.DockedWindowTypes);
    ...
    public void CreateGUI()
    {
        try
        {
            ...
            // 加载布局文件
            var visualAsset = UxmlLoader.LoadWindowUXML<ConfromBuilderWIndow>();
            if (visualAsset == null)
                return;
    ...
    private void RefreshBuildPipelineView()
        {
            ...
            {
                //实例化Viewer
                var viewer = new ConfromScriptableBuildPipelineViewer(_buildPackage, buildTarget, _container);
            }
